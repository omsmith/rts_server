...
public class Unit_Location_otherimp{
	...
	public boolean add(Position Posn, Integer Unique_ID){
		...
		boolean result;
		result = unitPositionTablebyX.add(addedUnit);
		if(result == true){
			result = unitPositionTablebyY.add(addedUnit);
			if(result == false){
				unitPositionTablebyX.remove(addedUnit.getUnique_ID());
				return false;
			}
			else return true;
		}
	...
	public boolean remove(Integer Unique_ID){
		boolean resultA = unitPositionTablebyX.remove(Unique_ID);
		boolean resultB = unitPositionTablebyY.remove(Unique_ID);
		if(resultA && resultB)
			return true;
		else
			return false;
	...
	class Unit_SortedList{
		...
		public boolean add(Unit_Element addedUnit) {
